...
internal class SotwtmSupportBaseModule {
    ...
    @LibScope
    @Provides
    fun appLocale(sharedPreferences: SharedPreferences,
                  editor: SharedPreferences.Editor): ObservableField<Locale> =
            object : ObservableField<Locale>() {
                ...
                override fun get(): Locale =
                        sharedPreferences.getString(SotwtmSupportLib.PREF_KEY_APP_LOCALE, null)?.let {
                            ...
                            if (languageTags.isEmpty) AppHelpfulLocaleUtil.getDefaultLangFromSystemSetting(supportedLocales(sharedPreferences, editor).get())
                            else languageTags[0]
                        }
                                ?: AppHelpfulLocaleUtil.getDefaultLangFromSystemSetting(supportedLocales(sharedPreferences, editor).get())
                ...
                override fun set(value: Locale) {
                    ...
                    editor.putString(SotwtmSupportLib.PREF_KEY_APP_LOCALE, LocaleListCompat.create(AppHelpfulLocaleUtil.unify(value)).toLanguageTags())
                    editor.apply()
                    notifyChange()
                }
            }
    ...
    @LibScope
    @Provides
    fun supportedLocales(sharedPreferences: SharedPreferences,
                         editor: SharedPreferences.Editor): ObservableField<List<Locale>> =
            object : ObservableField<List<Locale>>(emptyList()) {
                @Synchronized
                override fun get(): List<Locale> {
                    val supportedLocalesString = sharedPreferences.getString(SotwtmSupportLib.PREF_KEY_SUPPORTED_LOCALES, null)
                ...
                override fun set(newLocales: List<Locale>?) {
                    ...
                    if (newLocales == null || newLocales.isEmpty()) {
                        ...
                        if (localeListString != sharedPreferences.getString(SotwtmSupportLib.PREF_KEY_SUPPORTED_LOCALES, null)) {
